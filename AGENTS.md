...
This file provides guidance to Codex (Codex.ai/code) when working with code in this repository. It mirrors CLAUDE.md — if you change one, change both.

---

## 🟡 TOURNAMENT STATUS
**WC 2026 KNOCKOUT PHASE IN PROGRESS** (Group Stage completed Jun 27; R32 done 16/16; R16 done 8/8; Quarter-finals done 4/4 as of 2026-07-12 — France-Spain and England-Argentina are the semi-finals; Final is Jul 19, 2026)
...
- **Current ops:** Full matchday cycle applies for every knockout round (no more `update_third_place_probs.py` 3×/day — that was group-stage-only). After `predict_live.py --export`, also run `export_knockout_bracket.py`, `run_knockout_oracle.py`, and `run_upset_agent.py` (see Quick Reference below). CI runs these steps idempotently and seeds agent caches from the published frontend JSON on clean checkouts.
...
**Oráculo de Eliminatorias (`src/knockout_oracle.py`, run via `scripts/run_knockout_oracle.py`) — solo QF/SF/Final:** voz premium separada del debate de 3 agentes y del Cazador. Panel de 4 especialistas independientes (Group Analyst, Tactical Scout, Sentiment Reader, **Especialista en Definiciones**) + Consenso, cada uno con su propia llamada a `deepseek-reasoner` (~5 llamadas/partido). Razona la eliminatoria completa: 90' → prórroga → penaltis → **quién avanza** (nunca deja empate como resultado final). Solo corre en `ORACLE_ROUNDS` (Quarter-final, Semi-final, Final, Match for third place); forward-only (omite cruces ya jugados). CI lo ejecuta después de resolver el bracket y reutiliza la copia publicada para no repetir llamadas. Salida: `knockout_oracle_predictions.json`.
...
- `predict_live.py`'s `load_fixture()` calls into `bracket.py` instead of skipping every placeholder match. Knockout predictions are generated **even after kickoff** — the anti-leakage cutoff excludes each match's own result from ELO/form, and (since 2026-07-09) the **Poisson component is refit per match-date** for retro predictions: the global pkl is fitted on all played matches including the one being predicted, so `fit_poisson_before_date()` refits with data strictly before midnight of the match day (n_iter=30, ~22s/fit, cached per date; converges to <0.6pp of the pipeline's 100-iter fit). Future matches keep the pkl — it's clean by construction for any future kickoff. Group-stage matches still skip if already played.
