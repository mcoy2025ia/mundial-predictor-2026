...
**Oráculo de Eliminatorias (`src/knockout_oracle.py`, run via `scripts/run_knockout_oracle.py`) — solo QF/SF/Final:** voz premium separada del debate de 3 agentes y del Cazador. Panel de 4 especialistas independientes (Group Analyst, Tactical Scout, Sentiment Reader, **Especialista en Definiciones**) + Consenso, cada uno con su propia llamada a `deepseek-reasoner` (~5 llamadas/partido). Razona la eliminatoria completa: 90' → prórroga → penaltis → **quién avanza** (nunca deja empate como resultado final). Solo corre en `ORACLE_ROUNDS` (Quarter-final, Semi-final, Final, Match for third place); forward-only (omite cruces ya jugados). Salida: `knockout_oracle_predictions.json`.
